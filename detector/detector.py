...
def detect_image (section: str, height: float, width: float, gray: np.ndarray, bgr: np.ndarray) -> (np.ndarray, float, float, int):
    ...
    top_ignore = sec.getfloat('top_ignore', fallback=0.0)
    bot_ignore = sec.getfloat('bot_ignore', fallback=0.0)
    left_ignore = sec.getfloat('left_ignore', fallback=0.0)
    right_ignore = sec.getfloat('right_ignore', fallback=0.0)
    ksize_x, ksize_y = map(int, sec.get('ksize_grid').split(','))
    sigma_x = sec.getfloat('sigma_x', fallback=1.0)
    sigma_y = sec.getfloat('sigma_y', fallback=sigma_x)
    dp = sec.getfloat('dp')
    ...
    overlap_thresh = sec.getfloat('overlap_thresh', fallback=0.6)
    tolerance = sec.getint('tolerance', fallback=50)
