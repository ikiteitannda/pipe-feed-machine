...
# 确保 pictures 目录存在
pics_dir = os.path.join(os.path.dirname(__file__), 'pictures')
...
class CameraWorker(QThread):
    ...
    def run(self):
        ...
        if cnt < self.index:
            # 未找到相机，退出线程
            self.logMessage.emit("[CameraWorker] 未检测到相机，停止运行")
            return
        ...
        cam.GevHeartbeatTimeout.set(3000)
        # 数据流配置
        stream = cam.data_stream[0]
        stream.set_acquisition_buffer_number(4)
        ...
        while self._running:
            try:
                ...
                try:
                    ...
                    self.logMessage.emit(f"[{self.section}] 采集图片失败，失败原因：{e}")
                    continue
                self.logMessage.emit(f"[{self.section}] 已获取图像")

                # 可选：保存原图文件 构造文件名：YYYYMMDD_节名.bmp
                if self.save_image and os.path.exists(pics_dir):
                    now = datetime.datetime.now()
                    date_str = now.strftime("%Y%m%d")
                    filename = f"{date_str}_{self.section}.bmp"
                    filepath = os.path.join(pics_dir, filename)
                    cv2.imwrite(filepath, buf)
                    self.logMessage.emit(f"[{self.section}] 原图像已保存")

                self.logMessage.emit(f"[{self.section}] 检测开始...")
                if self.section == '':
                    # 引入检测算法接口
                    from detector import detect_image
                    pix, tx, ty, circles = detect_image(self.section, gray, bgr)
                elif self.section == '':
                    # 引入检测算法接口
                    from detector import one_detect_image
                    pix, tx, ty, circles = one_detect_image()
                else:
                    # 引入检测算法接口
                    from detector import one_detect_image
                    pix, tx, ty, circles = one_detect_image()
                ...
                self.logMessage.emit(f"[{self.section}] 检测完成: 圆心=({tx},{ty}), 圆管数量={len(circles)}")

                # 坐标转换并发 PLC 信号
                rx, rz = self.transform_to_robot_position(tx, ty)
                if abs(rx) <= 200 and abs(rz) <= 500:
                    self.coordReady.emit(rx, rz, len(circles))
                else:
                    self.logMessage.emit(f"[{self.section}] 坐标越界，忽略发送: x={rx:.3f}, z={rz:.3f}")
